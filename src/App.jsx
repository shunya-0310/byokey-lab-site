...
function Edo1868Page({ onNavigate, path }) {
  ...
  useEffect(() => {
    try {
      ...
      if (savedGame && savedGame.state && Array.isArray(savedGame.messages) && savedGame.state.turns > 0) {
        ...
        setMessages(savedGame.messages.map(message => message.role === "government" ? {...message, text: governmentMessage(savedGame.state.governmentReview || {})} : message));
        if (!savedGame.endingId && !canContinueNegotiation(savedGame.state)) {
          const assessment = evaluateSettlement(savedGame.state);
  ...
  const returnFromSettlement = () => {
    if (settlementFlow?.settlementResult !== "NOT_READY") return;
  ...
  const proceedGovernmentDecision = () => {
    if (settlementFlow?.settlementResult !== "ACCEPTED") return;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      setState(reviewed);
      finishRun(determineGovernmentOutcome(reviewed), finalMessages, reviewed);
    }, 3600);
