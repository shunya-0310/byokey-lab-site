import { activeClauses, agreementVersion } from './edo1868-clauses.js';
...
export function describeSettlement(snapshot) {
  ...
  const reasons = [...new Set((snapshot.governmentReview?.findings || []).map(f=>f.message))];
  ...
  const government = governmentAccepted ? '新政府も、その約定を承認した。あなたが持ち帰った合意は、軍を止め、江戸を平和に受け取るための決定となった。'
    : katsuAccepted ? `しかし、新政府は持ち帰られた約定を承認しなかった。会談での合意を、双方の軍勢を止める決定にはできなかった。${reasons.length ? '\n\n新政府が求めた修正は、次の点だった。\n'+reasons.join('\n') : ''}`
